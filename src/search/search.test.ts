...
import { vec, vectorize, vecsort, FrequencyIndex } from "./vec";
...
test("Trie", () => {
  let t = new Trie(["to", "today", "tomorrow", "toddler", "frog"]);
  ...
  let [depth, res] = t.find("organized");
  expect(depth).toBe(7);
  expect(res).toBe("organiz");
  t.find("be");
});
...
test("misc", () => {
  ...
  let docs = rawdocs.map((d: string) => {
    return { document: () => d, setRank: (r: number) => {} };
  });
  ...
  // query = "network money";
  query = "analysis";
  let rankings = store.search(query);
